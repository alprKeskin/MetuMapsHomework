...
class Graph
{
    private:
        ...
        // MY HELPER FUNCTIONS
        int FindVertexId(const std::string& vertexName) const;
        bool IsAllFalse(const std::vector<bool> givenVector) const;
        int FindMinimumUnvisitedVertex(const std::vector<int> distances, const std::vector<bool> unvisited) const;
        int FindEdgeBetweenTwoVertices(const int vertexId0, const int vertexId1) const;
        int FindHighestWeightedEdgeIndexOfPath(const std::vector<int> lastFoundPath) const;
